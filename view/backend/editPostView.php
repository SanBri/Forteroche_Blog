...
<div class="createPost">
    <form enctype="multipart/form-data" action='Index.php?action=updatePost&amp;id=<?= $post['id'] ?>&amp;img=<?= $post['img'] ?>&amp;token=<?= $_SESSION['token'] ?>' method="post">
        <p><strong>Titre de l'article : </strong></br><input type="text" value="<?= $post['title'] ?>" name="title" maxlength=50 size=150 required></p>
        ...
        <p><img src="/public/images/posts_img/<?= $post['img'] ?>" alt="<?= $post['img'] ?>" width="200"></p>
